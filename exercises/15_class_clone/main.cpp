...
public:
    ...
    // TODO: 实现复制构造器
    DynFibonacci(DynFibonacci const &params) : cache(new size_t[params.cached]), cached(params.cached) {
        // std::memcpy(cache, params.cache, cached * sizeof(size_t));
        for (int index = 0; index < cached; ++index) {
            cache[index] = params.cache[index];
        }
    }
